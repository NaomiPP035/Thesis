...
$('#export').onclick=()=>{const url=URL.createObjectURL(new Blob([JSON.stringify({status:'AI-drafted fictional examples, not research observations',entries},null,2)],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download='sensory-atlas-fictional-examples.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)};
...
function syncSaved(){let added=false;for(const r of readNewRecords()){entries.push(importRecord(r));const z=rand()*2-1,a=rand()*Math.PI*2,radius=Math.cbrt(rand()),ring=Math.sqrt(1-z*z);positions.push({x:radius*ring*Math.cos(a),y:radius*ring*Math.sin(a),z:radius*z,depth:.25+rand()*.75,phase:rand()*Math.PI*2});added=true}if(added){reset();draw()}}
